...
class TicTacToe(QMainWindow):
    def __init__(self):
        ...
        self.main_window.pvc.setChecked(True)
        self.game_type()
        for i in range(3):
            for j in range(3):
                self.buttons[i][j].clicked.connect(partial(self.play, i, j))
        self.main_window.new_game.clicked.connect(self.reset_game)
        self.main_window.about.clicked.connect(self.show_about)
        self.main_window.pvc.toggled.connect(self.game_type)
        self.main_window.pvp.toggled.connect(self.game_type)
    ...
    def game_type(self):
        ...
        self.player_2_score = 0
        self.reset_game()
        if self.main_window.pvc.isChecked():
            self.mode = "CPU"
        elif self.main_window.pvp.isChecked():
            self.mode = "Player"
        self.show_turn()
        self.show_score()
    ...
    def show_turn(self):
        if self.sign == "X":
            self.main_window.turn.setText(f"{self.sign} TURN")
        else:
            self.main_window.turn.setText(f"{self.sign} TURN")
    
    def show_score(self):
        self.main_window.x_board.setText(f"X (YOU)\n{self.player_1_score}")
        self.main_window.draw.setText(f"TIE\n{self.tie}")
        if self.mode == "CPU":
            self.main_window.o_board.setText(f"O (CPU)\n{self.cpu_score}")
        elif self.mode == "Player":
            self.main_window.o_board.setText(f"O (PLAYER)\n{self.player_2_score}")

    def play(self, row, col):
        if self.buttons[row][col].text() == "":
            if self.sign == "X":
                self.buttons[row][col].setText(self.sign)
                self.buttons[row][col].setStyleSheet("color: red; background-color: white")
                self.check_game()
                self.sign = "O"
                self.show_turn()
                if self.mode == "CPU":
                ...
                self.buttons[row][col].setText(self.sign)
                self.buttons[row][col].setStyleSheet("color: blue; background-color: white")
                self.check_game()
                self.sign = "X"
                self.show_turn()
    ...
    def check_game(self):
        if self.check_win() == True:
            ...
            if self.sign == "X":
                ...
                self.show_score()
                if self.mode == "CPU":
                    message_box.setText("You won!")
                else:
                    message_box.setText("Player 1 wins!")
            else:
                if self.mode == "CPU":
                    self.cpu_score += 1
                    self.show_score()
                    message_box.setText("CPU Won!")
                else:
                    self.player_2_score += 1
                    self.show_score()
                    message_box.setText("Player 2 Won!")
            message_box.exec()
            self.reset_game()
        else:
            if self.check_tie() == True:
                self.tie += 1
                self.show_score()
                message_box = QMessageBox(text = "It`s a tie")
